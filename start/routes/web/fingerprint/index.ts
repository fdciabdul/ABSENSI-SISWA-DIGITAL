...
router.group(() => {
    ...
    router.post('/fingerprint/:id/sync-users', [FingerprintController, 'syncUsers']).as('fingerprint.sync').middleware(middleware.role(['admin']))
    ...
    router.post('/fingerprint/:id/clear-logs', [FingerprintController, 'clearLogs']).as('fingerprint.clear').middleware(middleware.role(['admin']))
    router.post('/fingerprint/enroll', [FingerprintController, 'enrollFingerprint']).as('fingerprint.enroll').middleware(middleware.role(['admin']))
    ...
    router.delete('/fingerprint/:id', [FingerprintController, 'deleteFingerprint']).as('fingerprint.delete').middleware(middleware.role(['admin']))
