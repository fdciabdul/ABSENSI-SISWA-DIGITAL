...
router.group(() => {
  ...
  router.post('/devices', [DevicesController, 'store']).as('devices.store').middleware(middleware.role(['admin']))
  ...
  router.put('/devices/:id', [DevicesController, 'update']).as('devices.update').middleware(middleware.role(['admin']))
  router.delete('/devices/:id', [DevicesController, 'destroy']).as('devices.destroy').middleware(middleware.role(['admin']))
  ...
  router.post('/devices/:id/clear-logs', [DevicesController, 'clearLogs']).as('devices.clearLogs').middleware(middleware.role(['admin']))
